            ...
            if (m_bIsNote)
            {
                while (Value.IndexOf("<img ") > 0)
                {
                    ...
                    foreach (Match match1 in matches1)
                    {
                        ...
                        string strTargetPath = System.IO.Path.Combine(EAImporter.m_SharePath, strImagePackagePath);

                        string sourceFile = System.IO.Path.Combine(EAImporter.m_strImportFilePath, matchesSrc[0].Value);
                        sourceFile = sourceFile.Replace("/", "\\");

                        string strNewFileName = "W" + matchesWidth[0].Value + "_H" + matchesHeight[0].Value + "_ID" + matchesId[0].Value + "_AL" + strAlign + "_" + matchesSrc[0].Value.Split('/')[1];
                        ...
                        //string strNewFileName = matchesSrc[0].Value.Split('/')[1];
                        ...
                        {
                            throw new EpriException("Could not create directory for images: " + ex.Message, "");
                        ...
                        {
                            throw new EpriException("Error copying image to data folder. " + ex.Message, "");
                        }

                        string strReplacement = "&#13;&#10;<a href=\"" + destFile + "\"><font color=\"#0000ff\"><u>" + "<Ctrl Left Click with Mouse to View Image>" + "</u></font></a>&#13;&#10;";
        ...
        public int ParseAndValidateFields(string strParsingElementNote)
        {
            ...
            foreach (DictionaryEntry deFieldIndo in m_Fields)
            {
                ...
                if (!oFieldInfo.ParseAndValidateField())
                {
                    ...
                    throw new EpriException("\"" + oFieldInfo.ValueName + "\" missing or empty from " + strParsingElementNote + strCurrElement, oFieldInfo.XElement.getLocation());
