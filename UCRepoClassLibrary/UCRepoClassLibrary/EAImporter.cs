...
namespace UCRepoClassLibrary
{
    public class EAImporter
    {
        ...
        private Boolean m_bCloseEAFileOnExit = false;
        private Boolean m_bCloseEAOnExit = false;

        private static LogMsgCallbackType m_LogMsgCallback;
        private static LogMsgCallbackType m_ErrorMsgCallback;

        private static Word._Application m_oWord = null;
        ...
        public int Open(string strRepositoryFullPath)
        {
            ...
            if (System.Diagnostics.Debugger.IsAttached)
            {
                ...
                _strDataPath = Path.GetFullPath(ResourceFilePathPrefix) + @"\..\..\..\..\UCRepoClientApp\UCRepoClientApp\Resources\libtidy.dll";
        ...
        internal int ConvertWordTemplateToXML(string strPathToWordDoc, string strFileName, string strXSLTFullPath, string strXMLDataFile)
        {
            string strRootFileName = strFileName.Replace(".docx", "");
            string strWordFileName = strPathToWordDoc + "\\" + strFileName;
            string strWordToFilteredHTMLFileName = strPathToWordDoc + "\\" + strRootFileName + ".html";
            string strTidied = strPathToWordDoc + "\\" + strRootFileName + "_TIDY.xml";

            // Convert word doc to filtered HTML
            m_oWord.Visible = false;

            m_oWord.Documents.Open(strWordFileName);
            m_oWord.Visible = false;
        ...
        public int ImportWordDoc(string strPathToWordDoc, string strFileName, string strXSLTFullPath)
        {
            strPathToWordDoc = System.IO.Path.GetFullPath(strPathToWordDoc);
            strXSLTFullPath = System.IO.Path.GetFullPath(strXSLTFullPath);

            string strRootFileName = strFileName.Replace(".docx", "");
            string strXMLDataFile = strPathToWordDoc + "\\" + strRootFileName + "_MODEL.xml";


            if (!System.IO.File.Exists(strPathToWordDoc + "\\" + strFileName))
            {
            ...
            if (ConvertWordTemplateToXML(strPathToWordDoc, strFileName, strXSLTFullPath, strXMLDataFile) == 0)
            {
                return (ImportXMLFile(strPathToWordDoc, strRootFileName + "_MODEL.xml"));
            }
            else
            {
                return 1;
        ...
        public int CreateFolderForShare(string strTargetPath)
        {
            DisableCurrentShare();
        ...
        public int ImportXMLFile(string strPath, string strFile)
        {
            strPath = System.IO.Path.GetFullPath(strPath);

            if (!System.IO.File.Exists(strPath + "\\" + strFile))
            {
                LogError(LogErrorLevel.A, "Could not load xml document. File " + strPath + "\\" + strFile + " does not exist.");
                return 1;
            }
            ...
            string sBaselineGUID = SetupTempBaseline();
            if (sBaselineGUID == "")
            {
                m_Repository.BatchAppend = false;
                return 1;
            ...
            iErrorCount = iErrorCount + InitIKBStructure();
