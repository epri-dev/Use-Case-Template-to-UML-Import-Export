...
namespace UCRepoClassLibrary
{
    public class EAExporter
    {
        ...
        private static LogMsgCallbackType m_LogMsgCallback = null;
        private static LogMsgCallbackType m_ErrorMsgCallback = null;
        ...
        private EA.Package GetPackage(string strPackageFullPathName)
        {
        ...
        /// <returns></returns>
        XElement GetDomainLibraryForUseCase(EA.Package eaPackageComputation, EA.Package eaPackageEnterprise)
        {
        ...
        /// <param name="eaPackageComputation"></param>
        /// <returns></returns>
        XElement GetActorLibraryForUseCase(EA.Package eaPackageComputation, EA.Package eaPackageEnterprise,string strUseCaseName)
        {
            if (eaPackageComputation == null)
            {
                LogError(LogErrorLevel.A, "Could find Computation package");
        ...
        /// <param name="eaPackageComputation"></param>
        /// <returns></returns>
        XElement GetInfoExchangedForUseCase(EA.Package eaPackageComputation)
        {
            if (eaPackageComputation == null)
            {
                LogError(LogErrorLevel.A, "Could find Computation package");
        ...
        internal static int CheckForAndReplaceLinks(ref string strValue)
        {
            while (strValue.Contains("<a href=\""))
            {
                ...
                string strStyle = "";
                if ((strTemp.IndexOf("ST") + 2) == (strTemp.IndexOf("_") - 2))
                {
        ...
        private XElement GetUseCase(string strUseCasePackage)
        {
        ...
        /// 
        /// </summary>
        public int Export(string strUseCasePackage,string strXSLTFullPath, string strOutputFileName)
        {
            ...
            strOutputFileName = System.IO.Path.GetFullPath(strOutputFileName);

            string strRootFileName = Path.GetDirectoryName(strOutputFileName) + @"\" + Path.GetFileNameWithoutExtension(strOutputFileName);
            string strXMLDataFile = strRootFileName + "_MODEL.xml";
            ...
            m_strImageFileFullPath = strOutputFileName.Replace(Path.GetFileName(strOutputFileName), "") + Path.GetFileNameWithoutExtension(strOutputFileName) + "_files";

            m_strImageFileRelPath = Path.GetFileNameWithoutExtension(strOutputFileName) + "_files";
            ...
            XmlWriter writer = XmlWriter.Create(strOutputFileName, XWriterSettings);
